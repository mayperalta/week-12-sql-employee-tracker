...
const allDepartments = () => {
    return db.query("SELECT * FROM department ORDER BY department.id asc;", (err, res) => {
        if(err) throw err;
        console.table(res)
        userOptions();
    })
...
const allEmployees = () => {
    db.query(`
    SELECT employee.id, employee.first_name, employee.last_name, role.title,  role.id, role.salary, 
        department.name AS deparment, 
        CONCAT (manager.first_name, SPACE(1), manager.last_name) AS manager
        FROM employee
        LEFT JOIN role ON 
